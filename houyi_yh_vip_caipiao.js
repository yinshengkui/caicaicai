...
// http://www.888fy.com/lottery_cqssc.php

var flag_arr = [1];
var bei = 50;
var data_result_last3 = [6,4,7,1,2];
...
function myFunction100() {
    ...
    $.ajax({
        type: "POST",
        url: "http://www.880910.com/?controller=default&action=main",
        dataType: "json",
        data: {
            lotteryid: "1",
        ...
        success: function(data) {
            ...
            $("#right_05").find("div").eq(4).find(".btnList").find("li").eq(5).click(); //清除zhe一行
            console.log(data_result_last3);
            touzhu(data_result_last3[0], data_result_last3[1], data_result_last3[2], data_result_last3[3], data_result_last3[4]);
            $("#lt_sendok").click();
...
var touzhu_queding = function(data) {
        ...
        for (var i = 0; i < 10; i++) {
            data_result[i] = data.code[i].code.substring(4, 5);
        }
...
var touzhu = function(one, two, three, four, five) {
        // var result=[];
        $("#right_05").find("div").eq(4).find(".numList").find("li").eq(one).click();
        $("#right_05").find("div").eq(4).find(".numList").find("li").eq(two).click();
        $("#right_05").find("div").eq(4).find(".numList").find("li").eq(three).click();
        $("#right_05").find("div").eq(4).find(".numList").find("li").eq(four).click();
        $("#right_05").find("div").eq(4).find(".numList").find("li").eq(five).click();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (!flags) {
                ...
                for (var m = 0; m < data_result_last3.length; m++) {

                    if (data.code[0].code.substring(4, 5) == data_result_last3[m]) {
                        flag_arr_current = 1;
                        break;
                    } else {
                        continue;
